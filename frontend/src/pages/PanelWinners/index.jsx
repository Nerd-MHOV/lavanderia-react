...
const columns = [
  { field: 'collaborator', headerName: 'Colaborador', width: 400 },
...
const PanelWinners = () => {
  ...
  const getData = async () => {
    const api_response = await api.findOutputLog();
    makeLines(api_response)
    console.log(api_response)
  }

  const makeLines = async (data) => {
    data = data.filter(arr => arr.updatedAt > threeDay)
